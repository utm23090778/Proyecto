...
function App() {
  ...
  //validacion de pruguntas resueltas 
  ...
  if (questionsUnanswered.length > 0){
   ...
   try {
    ...
    Swal.fire("Respuestas alamacenadas", "", "success"). then(()=>window.location.reload)

   } catch (error) {
    Swal.fire( "No se pudieron guardar tus respuestas", error.msg, "error")
   }
  }
